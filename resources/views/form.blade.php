...
<div class="max-w-5xl mx-auto mt-6 px-4">
    <a href="{{ route('inventory.index') }}" class="inline-flex items-center gap-2 bg-[#003CBB] text-white px-6 py-3 rounded-xl shadow-sm hover:bg-[#003199] transition mb-8 text-lg font-medium">
    ...
    <h1 class="text-3xl font-bold text-[#111] mb-6">Add New Product</h1>
    ...
    <div class="bg-white p-8 rounded-2xl border border-gray-200 shadow-lg">
        <form action="{{ isset($item) ? route('inventory.update', $item->id) : route('inventory.store') }}" method="POST">
            ...
            <div class="mt-8 pt-6 border-t border-gray-100 text-right">
                <button type="submit" class="bg-slate-900 text-white px-10 py-3.5 rounded-xl text-lg font-bold hover:bg-slate-800 shadow-md transition uppercase tracking-wider">
